...
This is a substantially tighter check than comparing resolution terms alone: it accounts for reliability on both sides rather than assuming the historical baseline is exactly calibrated. Approximating resolution_prior by the baseline's raw cross-sectional variance (Var(πᵢ) ≈ 0.0074, from Section 4.2's reported standard deviation of 0.086) would make that assumption implicitly. Reliability_prior (0.0026) shows why it fails: more than twice reliability_price (0.0011), the historical baseline is itself measurably less well-calibrated than the market price, beyond simply being less resolving. Accounting for reliability on both sides closes most of the gap between the model's prediction and the data.
...
### 3.5 Two predictions of one equilibrium

Propositions 1 and 3 are two faces of the same τ. When informed trading dominates noise trading (high τ), price is highly informative, and trading on the already-public price-versus-prior divergence earns nothing further in expectation, because a high-τ price has already impounded essentially everything the divergence could tell a trader. The paper's own results fit this pairing directly. The large, highly significant encompassing-regression coefficient (β₂ = 5.78, Section 4.3) is the empirical signature of a high-τ market, and the trading strategy's failure to clear conventional significance thresholds (Section 4.5) is exactly what Proposition 3 predicts a public-information strategy should find in such a market. Both results are two empirical angles on one equilibrium pattern.
...
This agreement is a plausibility check rather than a formal test, since Lo's formula assumes i.i.d. per-period returns. The actual date-aggregated P&L series has zero-P&L days from threshold-gated trading, a varying number of contributing events per date, and vol-targeted position sizing, none of which the plain i.i.d. formula accounts for. The block bootstrap makes no i.i.d. assumption and remains this paper's primary uncertainty estimate. The analytic and simulated cross-checks corroborate its width using two independent, simpler methods.

### 3.7 Scope and limitations of the model

This model is deliberately stylized. Four simplifications bound how far its predictions should be read. It idealizes a bounded probability as an unbounded Gaussian, a standard approximation in this literature that breaks down near the boundaries of [0,1]. It treats the informativeness parameter τ as homogeneous across an empirically heterogeneous panel, when sectors with very different sample sizes and market structures (Section 4.2) plausibly have different τ. It takes the informed-trading intensity β as a reduced-form primitive, rather than deriving it from an informed trader's own risk-averse optimization, the natural next step for a fuller microfoundation. And it connects "zero expected profit in probability space" to "zero expected profit in the realized backtest" through an unproven, if reasonable, monotonicity assumption about how earnings surprises map to stock returns. None of these simplifications change the qualitative prediction that informativeness and public-information non-exploitability are linked facts, but they mean the model is an organizing framework for the paper's two empirical results rather than a structurally estimated description of Polymarket's actual market microstructure.

## Data and Methodology

### 4.1 Data

The core dataset is drawn from Polymarket's public Gamma and CLOB APIs, both unauthenticated and free to query. Polymarket tags every "Will [company] beat quarterly earnings?" contract under a single category, tag ID 1013, labeled "Earnings." We enumerate all such contracts using cursor-based pagination and retain the 1,231 that resolved between June 2025 and August 2026, spanning roughly 415 distinct tickers. One contract in the raw pull is mistagged: a market on the revenue from a viral social media post that happens to share the "Earnings" tag. Excluding it, the earliest genuine earnings-beat contract in the data resolves in June 2025, consistent with Polymarket having scaled this product only recently.

Each contract's title and description follow one of a small number of auto-generated templates that state the company, the scheduled earnings date, the consensus non-GAAP or GAAP EPS estimate, and the resolution criterion, and we extract the ticker, accounting basis, scheduled date, and consensus estimate from this text using a small set of regular expressions validated against the full sample. Of the 1,275 raw contracts, all but the mistagged one described above parse successfully (1,274 of 1,275), and because parsing and resolution are independent filters, the two headline counts in this section reconcile exactly. Forty-three of the 1,274 parseable contracts are still open, scheduled for earnings prints beyond the data-pull window, leaving the remaining 1,231 with a settled, unambiguous outcome that forms the analysis sample used everywhere else in this paper.

For each contract we also pull the full implied-probability price history for the "Yes" token from Polymarket's CLOB API, along with the underlying stock's daily price history and sector, industry, and market-capitalization metadata from Yahoo Finance. Nine of 424 unique tickers, affecting 16 of the 1,231 events, could not be resolved in Yahoo Finance at all, plausibly reflecting delistings, acquisitions, or renamings in the roughly 18 months since our knowledge of these companies was last current, and are treated as missing rather than imputed. A tenth ticker, BBBY, resolved a full price history but carries no sector or industry classification, likely a residual effect of its 2023 bankruptcy and delisting; its 4 events are included everywhere a sector label is not required, but excluded from the sector breakdown in Table 1. Together these ten tickers account for the 20 events missing from Table 1's eleven named sectors.

Polymarket's coverage of this product grew substantially over the sample period: from a single contract in the first partial quarter (2025 Q2), to 17 the following quarter, to a peak of 375 in the quarter ending December 2025 (2025 Q4), then 296, 319, and 223 in the three quarters since (below). This is a young product whose coverage is still scaling, which matters for how much weight the trading-strategy results in Section 4.5 can bear. The same pattern shows up in a rough proxy for market liquidity and data density: the number of recorded price ticks per contract rose in step, from roughly 50 in the earliest quarter to a range of 165 to 305 in every quarter since (below).
...
The estimated coefficient is small and not statistically significant: γ = 0.0104, standard error 0.0107, p = 0.330, n = 1,104 (below). We read this as an informative null result: the stock's own pre-announcement price action does not detectably track Polymarket's implied-probability momentum, consistent with the encompassing regression's finding that momentum itself carries no incremental information about the outcome. Section 4.5 turns from this informational question to whether the market's price level, which does carry information, translates into a tradable strategy.
...
Max drawdown here is not the conventional percentage-of-peak-equity ratio, and it is not bounded at -1. It sums risk-scaled per-event (or per-date) returns cumulatively rather than compounding them into a running NAV, then reports the largest gap between that cumulative sum and its own running peak. Position sizing leverages up to 3x on low-volatility stocks, so a run of levered losing events can push the cumulative sum more than 100 percentage points below its peak. A drawdown past -1 in this table means the strategy's summed risk-scaled losses exceeded its running peak by more than one full unit of risk budget, not that it lost more than all its capital.
